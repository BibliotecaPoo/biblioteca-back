...
namespace Biblioteca.Core.Authorization;
...
public static class CustomAuthorization
{
    public static bool ValidateUserClaims(HttpContext context, string claimName, string claimValue)
        => context.User.Identity!.IsAuthenticated && context.User.VerificarPermissao(claimName, claimValue);

    public static bool ValidateUserType(HttpContext context, string claimValue)
        => context.User.Identity!.IsAuthenticated && context.User.ObterTipoUsuario() == claimValue;
}

public class ClaimsAuthorizeAttribute : TypeFilterAttribute
{
    public ClaimsAuthorizeAttribute(string claimName, EPermissaoTipo claimValue) : base(typeof(RequirementClaimFilter))
    {
